...
from util import read_lp, write_csv, format
...
while ret:
    ...
    if ret and frame_nmr <10:
        ...
        # crop license plate and decal
        lp_crop = frame[int(detect_lp[-1][1]):int(detect_lp[-1][3]),int(detect_lp[-1][0]):int(detect_lp[-1][2]), :]
        decal_crop = frame[int(detect_decal[-1][1]):int(detect_decal[-1][3]), int(detect_decal[-1][0]):int(detect_decal[-1][2]), :]
